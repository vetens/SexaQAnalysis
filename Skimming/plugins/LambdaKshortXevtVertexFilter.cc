#include "LambdaKshortXevtVertexFilter.h"
using namespace reco;
using namespace edm;
using namespace std;

LambdaKshortXevtVertexFilter::LambdaKshortXevtVertexFilter(edm::ParameterSet const& pset):
  //collections
...
//These two functions are required to prevent a segfault at Run Transitions and File Transitions
void LambdaKshortXevtVertexFilter::endRun(Run const&, EventSetup const&) {
  //std::cout << "XEV: endofrun; clearing static vectors: " << 
  //  kshortKinFittedPrevEvent.size() << "=" <<
  //  kshortKinFittedVertexPrevEvent.size() << std::endl;
  kshortKinFittedPrevEvent_collection.clear();
  kshortKinFittedVertexPrevEvent_collection.clear();
  //std::cout << "XEV: endofrun; static vectors cleared." << std::endl;
}


void LambdaKshortXevtVertexFilter::respondToCloseInputFile(FileBlock const&) {
  //std::cout << "XEV: eof; clearing static vectors: " << 
  //  kshortKinFittedPrevEvent.size() << "=" <<
  //  kshortKinFittedVertexPrevEvent.size() << std::endl;
  kshortKinFittedPrevEvent_collection.clear();
  kshortKinFittedVertexPrevEvent_collection.clear();
  //std::cout << "XEV: eof; static vectors cleared." << std::endl;
}
...
//the real filter
bool LambdaKshortXevtVertexFilter::filter(edm::Event & iEvent, edm::EventSetup const & iSetup)
{ 
  ...
  for (unsigned int l = 0; l < h_lambda->size(); ++l) {
    //print the momenta from the lambdas f
...
//	cout << "Lambda tree not succesfully build" << endl; 
  ...
  for(unsigned int k = 0; k < h_kshort->size(); ++k){
    ...
    //cout << "LambdaKshortXevtVertexFilter: kshort momenta: " <<  (*h_kshort)[k]->px() << " " << (*h_kshort)[k]->py() << " " << (*h_kshort)[k]->pz() << endl;
...
//
//      RefCountedKinematicParticle lambda_l = lambdaKinFitted.at(l);
//      RefCountedKinematicParticle kshort_k = kshortKinFitted.at(k);
//      RefCountedKinematicVertex V_lambda_l = lambdaKinFittedVertex.at(l);
//      RefCountedKinematicVertex V_kshort_k = kshortKinFittedVertex.at(k);
//
...
//cross-event S/Sbar reconstruction
      for (unsigned int l = 0; l < lambdaKinFitted.size(); ++l) {
        for(unsigned int e = 0; e < kshortKinFittedPrevEvent_collection.size(); ++e){
            for(unsigned int k = 0; k < kshortKinFittedPrevEvent_collection.at(e).size(); ++k){
    
              //std::cout << "l, k = " << l <<  ", " << k << std::endl;
              RefCountedKinematicParticle lambda_l = lambdaKinFitted.at(l);
              RefCountedKinematicParticle kshort_k = kshortKinFittedPrevEvent_collection.at(e).at(k);
              RefCountedKinematicVertex V_lambda_l = lambdaKinFittedVertex.at(l);
              RefCountedKinematicVertex V_kshort_k = kshortKinFittedVertexPrevEvent_collection.at(e).at(k);
              int cProton =  chargeProton[l];
              reco::VertexCompositeCandidate S = FitS(lambda_l,kshort_k,V_lambda_l,V_kshort_k,cProton);
    ...
    //	    std::cout << "S candidate mass prev event " << S.mass() << std::endl;
        ...
        }//end loop over kshort collection from prev events
      }//end loop over lambda
     
      //std::cout << "saving " << kshortKinFitted.size() << " Ks for use in the next event" << std::endl;
    
      kshortKinFittedPrevEvent_collection.push_back(kshortKinFitted); 
      kshortKinFittedVertexPrevEvent_collection.push_back(kshortKinFittedVertex);
      if (kshortKinFittedPrevEvent_collection.size() > 50) {
        kshortKinFittedPrevEvent_collection.erase(kshortKinFittedPrevEvent_collection.begin());
        kshortKinFittedVertexPrevEvent_collection.erase(kshortKinFittedVertexPrevEvent_collection.begin());
      }
      int nsXev = sParticlesXEvent->size();
...
reco::VertexCompositeCandidate LambdaKshortXevtVertexFilter::FitS(RefCountedKinematicParticle lambdaKinFitted, RefCountedKinematicParticle kshortKinFitted, RefCountedKinematicVertex lambdaKinFittedVertex, RefCountedKinematicVertex kshortKinFittedVertex, int cProton){
...
bool LambdaKshortXevtVertexFilter::allCollectionValid(edm::Handle<reco::CandidatePtrVector> h_lambda,edm::Handle<reco::CandidatePtrVector> h_kshort){
  if(!h_lambda.isValid()) {
      std::cout << "Missing collection during LambdaKshortXevtVertexFilter: " << lambdaCollectionTag_   << " ... skip entry !" << std::endl;
      return false;
  }
  else if(!h_kshort.isValid()) {
      std::cout << "Missing collection during LambdaKshortXevtVertexFilter: " << kshortCollectionTag_   << " ... skip entry !" << std::endl;
      return false;
  }
...
bool LambdaKshortXevtVertexFilter::checkRefCountedKinematicTree(RefCountedKinematicTree Tree){
  if(Tree){
...
RefCountedKinematicTree LambdaKshortXevtVertexFilter::KinfitTwoTTracks(TransientTrack ttrack1, TransientTrack ttrack2, ParticleMass trackMass1, float trackMassSigma1, ParticleMass trackMass2, float trackMassSigma2, ParticleMass combinedMass, float combinedMassSigma){ 
  //making particles
...
RefCountedKinematicParticle LambdaKshortXevtVertexFilter::getTopParticleFromTree(RefCountedKinematicTree Tree){
  Tree->movePointerToTheTop();
  return  Tree->currentParticle();
}


RefCountedKinematicVertex LambdaKshortXevtVertexFilter::returnVertexFromTree(const RefCountedKinematicTree& myTree) const
{
  if (!myTree->isValid()) {
...
DEFINE_FWK_MODULE(LambdaKshortXevtVertexFilter);

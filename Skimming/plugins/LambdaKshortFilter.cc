...
bool LambdaKshortFilter::filter(edm::Event & iEvent, edm::EventSetup const & iSetup)
{
  ...
  if (isData_) {
    ...
    if(!h_bs.isValid()) {
      std::cout << "Missing collection during LambdaKshortFilter : " << bsCollectionTag_ << " ... skip entry !" << std::endl;
      return false;
    }
    ...
    if(!h_offlinePV.isValid()) {
      std::cout << "Missing collection during LambdaKshortFilter : " << PVCollectionTag_ << " ... skip entry !" << std::endl;
      return false;
    }

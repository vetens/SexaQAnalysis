...
collections_to_keep = cms.untracked.vstring(
    ...
    #'keep *_*_*_SEXAQ',
#dro#pping collections from intermediate filter steps
    #'drop *_*_sParticles_*',
    #'keep *_sMassFilter_sParticles_*',
##Fo#llowing two kept in Signal MC, but not originally here, so I will add them for now
    #'keep *_lambdaKshortVertexFilter_sParticles_*',
    #'keep *_offlinePrimaryVertices_*_*',
##Fo#llowing two seem to be asked about by something, as I get an error without them
#   # 'keep *_pfIsolatedElectronsEI_*_*',
#   # 'keep *_pfIsolatedMuonsEI_*_*',
##th#e above two no longer seem to be in the EDM info for data..... strange, but I will try something which is of the same datatype since the muons and electrons don't actually seem to be used anywhere...
...
inlist = open("Test_MC_SignalSbarFiles.txt", "r")

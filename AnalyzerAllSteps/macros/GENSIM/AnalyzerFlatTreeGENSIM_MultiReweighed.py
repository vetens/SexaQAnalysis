...
plots_output_dir = "plots_GENSIM_MultiReweighed/Mass_"+args.mass+"/"
...
inputFile = 'MultiSbar_'+args.mass+'_Reweighed.root'
...
# Necessary because the NTuple referenced here doesn't seem to have stored the multi-to-single weights...
fM2SWeights = '/afs/cern.ch/work/w/wvetens/Sexaquarks/CMSSW_10_2_26/src/SexaQAnalysis/AnalyzerAllSteps/data/MultiToSingleSbar_'+args.mass+'GeV_EtaReweigh.txt'
weight_etaM2S = {line.split()[0] : line.split()[1] for line in open(fM2SWeights)}

#define where output should go
fOut = TFile(plots_output_dir + 'MacroOut_'+inputFile.rsplit('/', 1)[-1],'RECREATE')
fOutText = open('Reconstructability_'+'MultiSbar_'+args.mass+'_Reweighed.txt', 'w')
...
#for reconstructability studies
...
#for M2SCompare
h_before_vz_antiS = TH1F('h_before_vz_antiS','; absolute v_{z} #bar{S} interaction vertex (cm); #Entries/5cm',100,-250,250)
h_before_eta_antiS = TH1F('h_before_eta_antiS','; #eta #bar{S}; #Entries/0.1#eta',160,-8,8)
h_before_pt_antiS = TH1F('h_before_pt_antiS','; p_{T} #bar{S} (GeV/c); #Entries/0.1GeV/c',100,0,10)
h_before_pz_antiS = TH1F('h_before_pz_antiS','; |p_{z}| #bar{S} (GeV/c); #Entries/GeV/c',80,0,80)

h_after_vz_antiS = TH1F('h_after_vz_antiS','; absolute v_{z} #bar{S} interaction vertex (cm); #Entries/5cm',100,-250,250)
h_after_eta_antiS = TH1F('h_after_eta_antiS','; #eta #bar{S}; #Entries/0.1#eta',160,-8,8)
h_after_pt_antiS = TH1F('h_after_pt_antiS','; p_{T} #bar{S} (GeV/c); #Entries/0.1GeV/c',100,0,10)
h_after_pz_antiS = TH1F('h_after_pz_antiS','; |p_{z}| #bar{S} (GeV/c); #Entries/GeV/c',80,0,80)
...
#these are created here but filled later since they require info on the tracker hits
...
#    weight_factor = treeAllAntiS._S_event_weighting_factor_all[0]*treeAllAntiS._S_event_weighting_factor_PU_all[0]
    ...
    #weight_factor = treeAllAntiS._S_event_weighting_factor_all[0]*treeAllAntiS._S_event_weighting_factor_M2S_all[0]*treeAllAntiS._S_event_weighting_factor_PU_all[0]
    #for this NTuple, the M2S weight branch seems to be all ones... so pulling from our text file for now
    w_m2s = float(weight_etaM2S[str(truncate(treeAllAntiS._S_eta_all[0],1))])
    weight_factor = treeAllAntiS._S_event_weighting_factor_all[0]*w_m2s*treeAllAntiS._S_event_weighting_factor_PU_all[0]
        #We don't want the weight factor for our M2S reweighting in the Denominator or it will just cancel out for eta and vz and other correlated variables
        #Think of it this way: the Multi To Single reweighting is an aspect of our detector acceptance and branching ratio to favor S that look like from Single Sbar events
    weight_factor_NoM2S = treeAllAntiS._S_event_weighting_factor_all[0]*treeAllAntiS._S_event_weighting_factor_PU_all[0]
    #print("eta:\t"+str(treeAllAntiS._S_eta_all[0])+"\tWeight Factor:\t"+str(weight_factor)+"\tNoM2S Weight:\t"+str(weight_factor_NoM2S)+"\n")
    ...
    h_denom_vz_antiS_reconstructable.Fill(vz_interaction_antiS,weight_factor_NoM2S)
    h_denom_eta_antiS_reconstructable.Fill(treeAllAntiS._S_eta_all[0],weight_factor_NoM2S)
    h_denom_pt_antiS_reconstructable.Fill(treeAllAntiS._S_pt_all[0],weight_factor_NoM2S)
    h_denom_pz_antiS_reconstructable.Fill(abs(treeAllAntiS._S_pz_all[0]),weight_factor_NoM2S)

    h_before_pz_antiS.Fill(abs(treeAllAntiS._S_pz_all[0]),weight_factor_NoM2S)
    h_before_pt_antiS.Fill(treeAllAntiS._S_pt_all[0],weight_factor_NoM2S)
    h_before_eta_antiS.Fill(treeAllAntiS._S_eta_all[0],weight_factor_NoM2S)
    h_before_vz_antiS.Fill(vz_interaction_antiS,weight_factor_NoM2S)

    h_after_pz_antiS.Fill(abs(treeAllAntiS._S_pz_all[0]),weight_factor)
    h_after_pt_antiS.Fill(treeAllAntiS._S_pt_all[0],weight_factor)
    h_after_eta_antiS.Fill(treeAllAntiS._S_eta_all[0],weight_factor)
    h_after_vz_antiS.Fill(vz_interaction_antiS,weight_factor)
    #print "Before Weight Factor:", weight_factor_NoM2S, "\tAfter Weight Factor:", weight_factor
    #print treeAllAntiS._S_pt_all[0], treeAllAntiS._S_eta_all[0], vz_interaction_antiS, treeAllAntiS._S_pz_all[0]
...
for h in l_tprof:
    ...
    c.Write()    
#For Making comparison plots between reweighed and non-reweighed
l_kin = [h_after_vz_antiS,h_after_eta_antiS,h_after_pt_antiS,h_after_pz_antiS]
l_kin_noM2S = [h_before_vz_antiS,h_before_eta_antiS,h_before_pt_antiS,h_before_pz_antiS]
for i,h in enumerate(l_kin):
    c = ReadyCanvas("c_M2SCompare_"+h.GetName().replace("after_",""))
    legend = TLegend(0.7,0.85,0.9,0.9)
    h_noM2S = l_kin_noM2S[i]
    Max = max(h_noM2S.GetMaximum(),h.GetMaximum())
    Min = min(h_noM2S.GetMinimum(0),h.GetMinimum(0))
    h_noM2S.Draw("P E1")
    h_noM2S.SetLineColor(colours[1])
    h_noM2S.SetMarkerStyle(23)
    h_noM2S.SetMarkerColor(colours[1])
    h_noM2S.SetStats(0)
    legend.AddEntry(h_noM2S, "Multi #bar{S} per Event", "ep")
    h.Draw("P E1 SAME")
    h.SetLineColor(colours[0])
    h.SetMarkerStyle(22)
    h.SetMarkerColor(colours[0])
    ...
    h.GetXaxis().SetLabelSize(0.03)
    h.GetYaxis().SetLabelSize(0.03)
    legend.AddEntry(h, "Multi-to-Single Reweighed", "ep")
    c.SetLogy()
    h_noM2S.GetYaxis().SetRangeUser(Min * 0.5, Max * 10)
    h.GetYaxis().SetRangeUser(Min * 0.5, Max * 10)
    legend.Draw("SAME")
    CMSStyle.setCMSLumiStyle(c,11, lumiTextSize_=0.74)
    c.SaveAs(plots_output_dir+c.GetName()+".pdf")
    c.Write()    
...
h2_pt_AntiLambda_Pion_AntiProton = TH2F("h2_pt_AntiLambda_Pion_AntiProton",";p_{T} (GeV/c) #bar{#Lambda}^{0}-#pi^{+};p_{T} (GeV/c) #bar{#Lambda}^{0}-#bar{p}; Events/(0.1GeV/c*0.05GeV/c)",60,0,3,60,0,6)
h2_eta_AntiLambda_Pion_AntiProton = TH2F("h2_eta_AntiLambda_Pion_AntiProton",";#eta #bar{#Lambda}^{0}-#pi^{+};#eta #bar{#Lambda}^{0}-#bar{p}; Events/(0.1#eta*0.1#eta)",80, -4, 4, 80, -4, 4)
...
#    weight_factor = tree._S_event_weighting_factor[0]*tree._S_event_weighting_factor_PU[0]
    w_m2s = float(weight_etaM2S[str(truncate(tree._S_eta[0],1))])
    weight_factor = tree._S_event_weighting_factor[0]*w_m2s*tree._S_event_weighting_factor_PU[0]
    ...
    h2_pt_AntiLambda_Pion_AntiProton.Fill(tree._GEN_AntiLambda_Pion_pt[0],tree._GEN_AntiLambda_AntiProton_pt[0],weight_factor)
    h2_eta_AntiLambda_Pion_AntiProton.Fill(tree._GEN_AntiLambda_Pion_eta[0],tree._GEN_AntiLambda_AntiProton_eta[0],weight_factor)
...
fOutText.write('Mass Point: ' + args.mass + '\n')
...
fOutText.write('the unweighted antiS reconstructability: ' + str(nAntiSReconstructableUnweighted) + '/' + str(nAntiSTotalUnweighted) + '=' + str(float(nAntiSReconstructableUnweighted)/float(nAntiSTotalUnweighted)) + '\n')
...
fOutText.write('the weighted antiS reconstructability: ' + str(nAntiSReconstructable) + '/' + str(nAntiSTotal) + '=' + str(float(nAntiSReconstructable)/float(nAntiSTotal)))
...
lxymaxval = h_antiS_lxy_interaction_vertex.GetMaximum()
h_antiS_lxy_interaction_vertex.GetYaxis().SetRangeUser(0, lxymaxval*1.2)
...
for h in l_TH1F:
    ...
    if(h.GetSumw2N() == 0):
        h.Sumw2(kTRUE)
    h.Scale(1./h.Integral(), "width");
    h.GetYaxis().SetTitle(h.GetYaxis().GetTitle()+"/N_{ev}")
    if h.GetName() == "h_antiS_lxy_interaction_vertex":
        lxymaxval = h.GetMaximum()
        h.GetYaxis().SetRangeUser(0, lxymaxval*1.2)
        h.Draw("HIST")
    else:
        h.Draw("CL")
    h.GetXaxis().SetLabelSize(0.03)
...
l_TH2F = [h2_eta_pt_AntiLambda_Pion,h2_eta_pz_AntiLambda_Pion,h2_eta_p_AntiLambda_Pion,h2_eta_pt_AntiLambda_AntiProton,h2_eta_pz_AntiLambda_AntiProton,h2_eta_p_AntiLambda_AntiProton, h2_pt_AntiLambda_Pion_AntiProton, h2_eta_AntiLambda_Pion_AntiProton]
for h in l_TH2F:
    h.SetDirectory(0)
for h in l_TH2F:
    c= ReadyCanvas(h.GetName())
...
h_neutron_momentum.GetYaxis().SetTitle(h_neutron_momentum.GetYaxis().GetTitle()+"/N_{ev}")
...
for i,l in enumerate(ll_TH1F):
    ...
    for h in l:
        ...
        if(h.GetSumw2N() == 0):
            h.Sumw2(kTRUE)
            h.Scale(1./h.Integral(), "width");
        #Prevent double labeling of y-axes of histograms which are used twice in this loop
        if i != 6 and i != 7:
            h.GetYaxis().SetTitle(h.GetYaxis().GetTitle()+"/N_{ev}")
        h.SetLineColor(colours[i_l_TH1F])
        h.SetMarkerStyle(22+i_l_TH1F)

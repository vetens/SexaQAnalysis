...
plots_output_dir = "plots_GEN/MassComparison_NoEtaCut/"
...
for j, f in enumerate(fIn):
	...
	print 'Number of entries in the tree: ', nEntries
        print "input file", mass[j]
        nSingleSbar = 0
        nSbar = 0
        nSbarPosEta = 0
        nSbarNegEta = 0
        nSingleSbar_PassEta = 0
        nSingleSbarPosEta = 0
        nSingleSbarPosEta_PassEta = 0
        nSingleSbarNegEta = 0
        nSingleSbarNegEta_PassEta = 0
	for i in range(0,nEntries):
		...
		tree.GetEntry(i)
                #if(tree._S_charge[0] < 0):
                nSbar += 1
                if(tree._S_eta[0] < 0):
                    nSbarNegEta += 1
                if(tree._S_eta[0] > 0):
                    nSbarPosEta += 1
                if(tree._N_Sbar[0] > 1 or tree._S_charge[0] > 0): continue
                nSingleSbar += 1
                if(tree._S_eta[0] < 0):
                    nSingleSbarNegEta += 1
                if(tree._S_eta[0] > 0):
		...
		h_antiS_eta_pz.Fill(tree._S_eta[0],abs(tree._S_pz[0]))
                if(abs(tree._S_eta[0]) < 5):
                    nSingleSbar_PassEta += 1
                    if(tree._S_eta[0] < 0):
                        nSingleSbarNegEta_PassEta += 1
                    if(tree._S_eta[0] > 0):
                        nSingleSbarPosEta_PassEta += 1
	print 'Number of Sbar in the sample: ', nSbar
	print 'Number of Sbar with positive eta in the sample: ', nSbarPosEta
	print 'Number of Sbar with negitive eta in the sample: ', nSbarNegEta
	print 'Number of Sbar from single Sbar events in the sample: ', nSingleSbar
	print 'Number of Sbar with positive eta from single Sbar events in the sample: ', nSingleSbarPosEta
	print 'Number of Sbar with negitive eta from single Sbar events in the sample: ', nSingleSbarNegEta
	print 'Number of Sbar from single Sbar events with eta < 5: ', nSingleSbar_PassEta
	print 'Number of Sbar with positive eta from single Sbar events passing this cut in the sample: ', nSingleSbarPosEta_PassEta
	print 'Number of Sbar with negitive eta from single Sbar events passing this cut in the sample: ', nSingleSbarNegEta_PassEta
	print 'Efficiency of this gen-level cut (n Single Sbar passing eta)/(n Single Sbar): ', float(nSingleSbar_PassEta)/float(nSingleSbar)
...
fOut = TFile('macro_FlatTree_GEN_Mass_Comparison_NoEtaCut.root','RECREATE')
